...
def main():
   ...
   shap_output_path = os.path.join(OUTPUT_DIR, "global", "shap")
   os.makedirs(shap_output_path, exist_ok=True)
   importance_sorted.to_csv(
       os.path.join(shap_output_path, "global_shap.txt"),
       index=False,
       sep='\t',
       float_format='%.4f'
   )
   ...
   four_collision_positions = []
   for position in range(len(y_test)):
       if y_test[X_test.index[position]] == 4:
           four_collision_positions.append(position)

   if len(four_collision_positions) == 0:
       print("No test cases found with specified collisions")
       return

   # Randomly select one of these positions
   position = np.random.choice(four_collision_positions)
