...
def main():
    ...
    print("\nInitial data type of Y_COLUMN:")
    print(model_df[Y_COLUMN].dtype)

    print("\nSample of initial data:")
    print(model_df[Y_COLUMN].head(10))

    print(model_df[Y_COLUMN].describe())
    model_df[Y_COLUMN] = ~model_df[Y_COLUMN]
    model_df[Y_COLUMN] = model_df[Y_COLUMN].astype(int)

    print("\nConverted side_move distribution:")
    print(model_df[Y_COLUMN].value_counts())
    ...
    print("\nSaving results")
    regression.save_model(best_model, 'best_sidemove_model')
    predictions.to_csv('out/explainability/sidemove_predictions.csv', index=False)
